...
class Tf_Idf_Model(object):
    def __init__(self, data, dictionary_filepath, stop_words_file, model_name='tfidf'):
        ...
        self.content_list = [line.rstrip().split() for line in self.content] # 分词后的二维数组
    ...
    def build_model(self, model_name):
        ...
        if self.model_name == 'lsi':
            ...
            idx = gensim.similarities.SparseMatrixSimilarity(
                tfidf[corpus], num_features=num_features)
        return idx, tfidf
    ...
    def get_topn_sims(self, sentences, n=5):
        ...
        for i in range(n):
            ...
            each_results_2 = {'index': str(topn_queries_num[i]+1), 'similarity': str(topn_values[i]), 'title': topn_queries[i],
                              'confidence': None}
...
#     parser.add_argument ('--dictionary_filepath', help='dictionary_filename',default=os.path.join(pathconfig.OTHER_DATA_PATH,'question_dictionary1.dict'), type=str)
